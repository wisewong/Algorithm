package wz.leetcode.problem53;

/**
 * Solution
 * DP问题
 * sub problem like: maxSubArray(int A[], int i),
 * which means the maxSubArray for A[0:i ] which must has A[i] as the end element.
 * @author wz
 * @date 2016年04月30日 21:59
 */
...
//        int best = Integer.MIN_VALUE,from = 0,to = 0;
//        for (int i = 0; i < nums.length; i++)
//            arr[i][i] = nums[i];
//        for (int i = 2; i < nums.length ; i++) {
//            for (int j = 0; j < nums.length - i +1; j++) {
//                arr[j][i+j-1] = Integer.MIN_VALUE;
//                for (int k = j; k < i+j-1; k++)
//                    if (arr[j][k]+arr[k+1][i+j-1]>arr[j][i+j-1])
//                        arr[j][i+j-1] = arr[j][k]+arr[k+1][i+j-1];
//                if (arr[j][i+j-1]>best){
//                    best = arr[j][i+j-1];
//                    from = j;
//                    to = i+j-1;
